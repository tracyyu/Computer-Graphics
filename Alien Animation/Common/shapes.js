...
inherit(cylindrical_strip, triangle_strip);
	...
	cylindrical_strip.prototype.populate = function( recipient, numRectangles, points_transform )	
				{	
					...
					for( var i = 0; i < numRectangles; i++ )
					{
						vertices[topIdx] 	= vec3( Math.cos(2 * Math.PI * topIdx / numRectangles), .5, Math.sin(2 * Math.PI * topIdx / numRectangles), 0.5 );	
						recipient.texture_coords[topIdx + offset]    = vec2(0, topIdx / numRectangles );
						vertices[bottomIdx] = vec3( Math.cos(2 * Math.PI * topIdx / numRectangles), -.5, Math.sin(2 * Math.PI * topIdx / numRectangles), -.5 );			
...
inherit(triangle, shape);
	triangle.prototype.populate = function( recipient, points_transform)
	{
		...
		recipient.normals = [ vec3(0,0,1), vec3(0,1,0), vec3(0,0,1) ];
...
inherit(windmill, shape);
 	windmill.prototype.populate = function( recipient, points_transform ) // The meat of the windmill starts here:
 	{
 		...
 		recipient.texture_coords.push( vec2( 0, 0 ) );
 			for( var i = 0, i_max = 361; i < i_max; i++ )
 			{
...
inherit(pyramid, shape);
	...
	var index_offset = recipient.indices.length;				
        ...
        recipient.vertices.push( vec3(0,1,0), vec3(-1,-1,1), vec3(1,-1,1) );
        recipient.vertices.push( vec3(0,1,0), vec3(1,-1,1), vec3(1,-1,-1) );
        recipient.vertices.push( vec3(0,1,0), vec3(1,-1,-1), vec3(-1,-1,-1) );
        recipient.vertices.push( vec3(0,1,0), vec3(-1,-1,-1), vec3(-1,-1,1) );
		
		
        recipient.texture_coords.push( vec2(0,1), vec2(-1,-1), vec2(1,-1) ); 
		recipient.texture_coords.push( vec2(0,1), vec2(1,-1), vec2(1,-1) ); 
		recipient.texture_coords.push( vec2(0,1), vec2(1,-1), vec2(-1,-1) ); 
		recipient.texture_coords.push( vec2(0,1), vec2(-1,-1), vec2(-1,-1) ); 
...
function capped_cylinder() // Combine a tube and two flattened triangle fans to make a solid cylinder
 {
 	...
 	this.populate = ( function (self)
 	{
 		...
 		object_transform = mult( object_transform, translate(0, 0, .5));
...
function spike(points_transform) {
    shape.call(this);
    if (!arguments.length) return;
    this.populate(this, points_transform);
    this.init_buffers();
...
spike.prototype.populate = function (recipient, points_transform) {
    var offset = recipient.vertices.length;
    var index_offset = recipient.indices.length;

    // TODO: Implement parameters

    recipient.vertices.push(vec3(-1, 1, 0), vec3(0, 2, 0), vec3(1, 1, 0));
    recipient.vertices.push(vec3(-1, 1, 0), vec3(0, 0, 0), vec3(1, 1, 0));
    recipient.vertices.push(vec3(-2, 0, 0), vec3(0, 0, 0), vec3(-1, 1, 0));
    recipient.vertices.push(vec3(2, 0, 0), vec3(0, 0, 0), vec3(1, 1, 0));

    recipient.vertices.push(vec3(0, 1, -1), vec3(0, 2, 0), vec3(0, 1, 1));
    recipient.vertices.push(vec3(0, 1, -1), vec3(0, 0, 0), vec3(0, 1, 1));
    recipient.vertices.push(vec3(0, 0, -2), vec3(0, 0, 0), vec3(0, 1, -1));
    recipient.vertices.push(vec3(0, 0, 2), vec3(0, 0, 0), vec3(0, 1, 1));

    recipient.vertices.push(vec3(-1, 1, 1), vec3(0, 2, 0), vec3(1, 1, -1));
    recipient.vertices.push(vec3(-1, 1, 1), vec3(0, 0, 0), vec3(1, 1, -1));
    recipient.vertices.push(vec3(-2, 0, 1), vec3(0, 0, 0), vec3(-1, 1, -1));
    recipient.vertices.push(vec3(2, 0, 1), vec3(0, 0, 0), vec3(1, 1, -1));


    recipient.texture_coords.push(vec2(-1, 0), vec2(0, 1), vec2(1, 0));
    recipient.texture_coords.push(vec2(-1, 0), vec2(0, 1), vec2(1, 0));
    recipient.texture_coords.push(vec2(0, 0), vec2(0, 1), vec2(1, 0));
    recipient.texture_coords.push(vec2(0, 0), vec2(0, 1), vec2(1, 0));
    recipient.texture_coords.push(vec2(0, 0), vec2(0, -1), vec2(-1, 0));
